...
.DESCRIPTION
    ...
    - Offers automatic integration with Claude Desktop,  CLI, VSCode, Cursor, Windsurf, and Trae
...
function Cleanup-Docker {
    ...
    # Remove images
    $images = @("-mcp-server:latest", "zen-mcp-server:latest")
...
function Test-ApiKeys {
    ...
    # Patterns for API key validation (avoid duplicate hashtable keys)
    $openRouterPattern = "sk-or-[a-zA-Z0-9-_]+"
    $genericKeySuffix = "_API_KEY$"
    $genericPatterns = @(
        "AIza[0-9A-Za-z-_]{35}",
    ...
    foreach ($line in $envContent) {
        if ($line -match '^([^#][^=]*?)=(.*)$') {
            ...
            $placeholder = "your_{0}_here" -f ($key.ToLower())
            $valid = $false

            if ($key -eq "OPENROUTER_API_KEY") {
                if ($value -match $openRouterPattern) { $valid = $true }
            } elseif ($key -match $genericKeySuffix) {
                foreach ($pat in $genericPatterns) {
                    if ($value -match $pat) { $valid = $true; break }
                }
            }

            if ($valid -and $value -ne $placeholder -and $value.Length -gt 10) {
...
function Invoke-DockerImageBuild {
    ...
    if ($Force -and $imageExists) {
        ...
        try {
            docker rmi ex-mcp-server:latest 2>$null
        } catch {
...
_API_KEY=your__api_key_here
_API_KEY=your__api_key_here
_API_HOST=your__api_host_here
_API_VERSION=your__api_version_here
...
$script:McpClientDefinitions = @(
    @{
        ...
        ConfigJsonPath = "mcpServers.exai"
        NeedsConfigDir = $true
    },
    @{
        Name = "VSCode"
        ...
        ConfigJsonPath = "mcp.servers.exai"
        IsVSCode = $true
    },
    @{
...
$script:DockerMcpConfig = @{
    command = "docker"
    args    = @("exec", "-i", "ex-mcp-server", "python", "server.py")
    type    = "stdio"
}
...
function Get-DockerMcpConfigRun {
    ...
    return @{
        command = "docker"
        args    = @("run", "--rm", "-i", "--env-file", $envFile, "ex-mcp-server:latest", "python", "server.py")
        type    = "stdio"
    }
}
...
function Set-McpClientConfiguration {
    ...
    try {
        ...
        if (Test-Path $configPath) {
            Save-ConfigBackup -ConfigFilePath $configPath
        }
...
function Invoke-McpClientConfiguration {
    ...
    foreach ($client in $script:McpClientDefinitions) {
        Set-McpClientConfiguration -Client $client -UseDocker $UseDocker -PythonPath $PythonPath -ServerPath $ServerPath
    }
    
    # Handle CLI tools separately (they don't follow JSON config pattern)
    if (!$UseDocker) {
        Test-ClaudeCliIntegration $PythonPath $ServerPath
        Test-CliIntegration (Split-Path $ServerPath -Parent)
    }
}
...
function Test-CliIntegration {
    ...
    # Check if  settings file exists (Windows path)
    $Config = "$env:USERPROFILE\.\settings.json"
    if (!(Test-Path $Config)) {
        return
    }
    
    # Check if zen is already configured
    $configContent = Get-Content $Config -Raw -ErrorAction SilentlyContinue
    ...
    Write-Host ""
    $response = Read-Host "Configure Zen for  CLI? (y/N)"
    if ($response -ne 'y' -and $response -ne 'Y') {
        Write-Info "Skipping  CLI integration"
        return
    }
    
    # Ensure wrapper script exists
    if (!(Test-Path $zenWrapper)) {
        Write-Info "Creating wrapper script for  CLI..."
        @"
@echo off
cd /d "%~dp0"
...
"@ | Out-File -FilePath $zenWrapper -Encoding ASCII
    ...
    # Update  settings
    Write-Info "Updating  CLI configuration..."
    
    try {
        # Create backup with retention management
        $null = Save-ConfigBackup $Config
        ...
        $config | ConvertTo-Json -Depth 10 | Out-File $Config -Encoding UTF8
        
        Write-Success "Successfully configured  CLI"
        Write-Host "  Config: $Config" -ForegroundColor Gray
        Write-Host "  Restart  CLI to use Zen MCP Server" -ForegroundColor Gray
        
    } catch {
        Write-Error "Failed to update  CLI config: $_"
        Write-Host ""
        Write-Host "Manual config location: $Config"
...
function Invoke-DockerWorkflow {
    ...
    if (!(Invoke-DockerImageBuild -Force:$Force)) { exit 1 }
