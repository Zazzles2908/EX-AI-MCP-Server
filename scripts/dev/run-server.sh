...
cleanup_docker() {
    ...
    # Remove images
    local images=("-mcp-server:latest" "ex-mcp-server:latest")
...
install_dependencies() {
    ...
    local packages=("mcp" "google.genai" "" "pydantic" "dotenv")
    ...
    echo ""
    print_info "Setting up EX MCP Server..."
    ...
    if [[ $exit_code -ne 0 ]]; then
        ...
        if echo "$install_output" | grep -q "No module named pip"; then
            ...
            echo "  $python_cmd -m pip install mcp google-genai  pydantic python-dotenv"
        fi
        return 1
    else
...
check_claude_cli_integration() {
    ...
    if ! command -v claude &> /dev/null; then
        ...
        echo ""
        read -p "Would you like to add EX MCP to Claude Code? (Y/n): " -n 1 -r
        echo ""
        if [[ $REPLY =~ ^[Nn]$ ]]; then
    ...
    if echo "$mcp_list" | grep -q "exai"; then
        # Check if it's using the old Docker command
        if echo "$mcp_list" | grep -E "zen.*docker|zen.*compose" &>/dev/null; then
            print_warning "Found old Docker-based EX registration, updating..."
            claude mcp remove exai -s user 2>/dev/null || true

            # Re-add with correct Python command
            if claude mcp add exai -s user -- "$python_cmd" "$server_path" 2>/dev/null; then
                print_success "Updated EX to become a standalone script"
                return 0
            else
                echo ""
                echo "Failed to update MCP registration. Please run manually:"
                echo "  claude mcp remove exai -s user"
                echo "  claude mcp add exai -s user -- $python_cmd $server_path"
                return 1
            fi
        else
            ...
            if echo "$mcp_list" | grep -F "$server_path" &>/dev/null; then
                ...
                print_warning "EX registered with different path, updating..."
                claude mcp remove exai -s user 2>/dev/null || true

                if claude mcp add exai -s user -- "$python_cmd" "$server_path" 2>/dev/null; then
                    print_success "Updated EX with current path"
                    return 0
                else
                    echo ""
                    echo "Failed to update MCP registration. Please run manually:"
        ...
        echo ""
        read -p "Add EX MCP to Claude Code? (Y/n): " -n 1 -r
        echo ""
        if [[ $REPLY =~ ^[Nn]$ ]]; then
...
check_claude_desktop_integration() {
    ...
    echo ""
    read -p "Configure EX MCP for Claude Desktop? (Y/n): " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Nn]$ ]]; then
    ...
    if [[ -f "$config_path" ]]; then
        ...
        if grep -q "docker.*compose.*exai\|exai.*docker" "$config_path" 2>/dev/null; then
...
try:
    ...
    # Remove exai from mcpServers if it exists (cleanup old entries)
    if 'mcpServers' in config and 'exai' in config['mcpServers']:
        del config['mcpServers']['zen']
        print('Removed old EX MCP configuration')
...
# Check and update  CLI configuration
check__cli_integration() {
    local script_dir="$1"
    local zen_wrapper="$script_dir/ex-mcp-server"

    # Check if  settings file exists
    local _config="$HOME/./settings.json"
    if [[ ! -f "$_config" ]]; then
    ...
    if grep -q '"exai"' "$_config" 2>/dev/null; then
        # Already configured
        return 0
    fi

    # Ask user if they want to add Zen to  CLI
    echo ""
    read -p "Configure EX MCP for  CLI? (Y/n): " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Nn]$ ]]; then
        print_info "Skipping  CLI integration"
        return 0
    fi

    # Ensure wrapper script exists
    if [[ ! -f "$zen_wrapper" ]]; then
        print_info "Creating wrapper script for  CLI..."
        cat > "$zen_wrapper" << 'EOF'
#!/bin/bash
# Wrapper script for  CLI compatibility
DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$DIR"
exec .venv/bin/python server.py "$@"
EOF
        chmod +x "$zen_wrapper"
        print_success "Created ex-mcp-server wrapper script"
    fi

    # Update  settings
    print_info "Updating  CLI configuration..."

    # Create backup
    cp "$_config" "${_config}.backup_$(date +%Y%m%d_%H%M%S)"

    # Add EX MCP configuration using Python for proper JSON handling
    local temp_file=$(mktemp)
    python3 -c "
import json
import sys
...
try:
    ...
    print(f'Error processing config: {e}', file=sys.stderr)
    sys.exit(1)
" && mv "$temp_file" "$_config"

    if [[ $? -eq 0 ]]; then
        print_success "Successfully configured  CLI"
        echo "  Config: $_config"
        echo "  Restart  CLI to use EX MCP Server"
    else
        print_error "Failed to update  CLI config"
        echo "Manual config location: $_config"
        echo "Add this configuration:"
        cat << EOF
{
  "mcpServers": {
...
display_config_instructions() {
    ...
    # Get script directory for  CLI config
    local script_dir=$(dirname "$server_path")

    echo ""
    local config_header="EX MCP SERVER CONFIGURATION"
    echo "===== $config_header ====="
    printf '%*s\n' "$((${#config_header} + 12))" | tr ' ' '='
    echo ""
    echo "To use EX MCP Server with your Claude clients:"
    echo ""

    print_info "1. For Claude Code (CLI):"
    echo -e "   ${GREEN}claude mcp add exai -s user -- $python_cmd $server_path${NC}"
    echo ""
...
EOF
    ...
    print_info "For  CLI:"
    echo "   Add this configuration to ~/./settings.json:"
    echo ""
    cat << EOF
   {
     "mcpServers": {
...
display_setup_instructions() {
    ...
    printf '%*s\n' "$((${#setup_header} + 12))" | tr ' ' '='
    echo ""
    print_success "EX MCP is ready to use!"
}
...
show_help() {
    ...
    local header="🤖 EX MCP Server v$version"
    echo "$header"
    printf '%*s\n' "${#header}" | tr ' ' '='
    echo ""
...
main() {
    ...
    # Display header
    local main_header="🤖 EX MCP Server"
    echo "$main_header"
    printf '%*s\n' "${#main_header}" | tr ' ' '='
    ...
    # Step 10: Check  CLI integration
    check__cli_integration "$script_dir"
