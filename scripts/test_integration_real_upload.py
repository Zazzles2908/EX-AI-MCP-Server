...
if env_path.exists():
    ...
    print(f"[WARN] .env.docker not found at {env_path}")
...
def check_environment():
    ...
    for var, description in required_vars.items():
        ...
        if value:
            ...
            print(f"[OK] {var}: {masked} ({description})")
        else:
...
def create_test_files():
    ...
    print(f"[OK] Created small file: {small_file} ({small_file.stat().st_size} bytes)")
    ...
    print(f"[OK] Created medium file: {medium_file} ({medium_file.stat().st_size} bytes)")
    ...
    print(f"[OK] Created large file: {large_file} ({large_file.stat().st_size} bytes)")
...
async def test_kimi_gateway(test_file):
    ...
    try:
        ...
        if not storage or not storage.enabled:
            print("[FAIL] Supabase storage not enabled")
            return False

        print(f"[FILE] Uploading file: {test_file}")
        ...
        if db_result.data:
            print(f"[OK] Verified in database: {len(db_result.data)} record(s)")
        else:
            print(f"[WARN] Not found in database")
...
async def test_size_validator(test_files):
    ...
    try:
        ...
        for name, file_path in test_files.items():
            ...
            print(f"\n[FILE] {name.upper()} FILE: {file_path.name}")
