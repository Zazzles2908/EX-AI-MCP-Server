# MiniMax M2-Stable Configuration Verification Script
# Created: 2025-11-04
# Purpose: Verify Claude Code is properly configured to use MiniMax M2-Stable
...
Write-Host "MiniMax M2-Stable Configuration Verification" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
if ($Config.modelConfig) {
    ...
    # Check primary model
    if ($Config.modelConfig.primaryModel -eq "MiniMax-M2-Stable") {
        Write-Status "Primary model: MiniMax-M2-Stable" "PASS" "Green"
    } else {
        Write-Status "Primary model: $($Config.modelConfig.primaryModel) (Expected: MiniMax-M2-Stable)" "FAIL" "Red"
        $ErrorCount++
    }

    # Check fallback model
    if ($Config.modelConfig.fallbackModel -eq "MiniMax-M2-Stable") {
        Write-Status "Fallback model: MiniMax-M2-Stable" "PASS" "Green"
    } else {
        Write-Status "Fallback model: $($Config.modelConfig.fallbackModel) (Expected: MiniMax-M2-Stable)" "FAIL" "Red"
        $ErrorCount++
    }
    
    # Check model identifier
    if ($Config.modelConfig.model -eq "minimax-m2-stable") {
        Write-Status "Model identifier: minimax-m2-stable" "PASS" "Green"
    } else {
        Write-Status "Model identifier: $($Config.modelConfig.model) (Expected: minimax-m2-stable)" "WARN" "Yellow"
        $WarningCount++
    }
} else {
...
if ($Config.modelConfig.env) {
    ...
    if ($Config.modelConfig.env.ANTHROPIC_MODEL -eq "MiniMax-M2-Stable") {
        Write-Status "ANTHROPIC_MODEL: MiniMax-M2-Stable" "PASS" "Green"
    } else {
...
if ($Config.modelConfig.modelOverrides) {
    ...
    $ExpectedOverrides = @{
        "claude-3-5-haiku-20241022" = "minimax-m2-stable"
        "claude-3-5-sonnet-20241022" = "minimax-m2-stable"
        "claude-3-opus-20240229" = "minimax-m2-stable"
    }
...
if ($env:MINIMAX_M2_KEY) {
    Write-Status "MINIMAX_M2_KEY is set" "PASS" "Green"
    if ($Verbose) {
        Write-Status "Value: $($env:MINIMAX_M2_KEY.Substring(0, [Math]::Min(10, $env:MINIMAX_M2_KEY.Length)))..." "INFO" "Cyan"
    }
} else {
    Write-Status "MINIMAX_M2_KEY is NOT set" "WARN" "Yellow"
    Write-Host "  Set with: `$env:MINIMAX_M2_KEY = 'your-api-key-here'" -ForegroundColor Gray
    $WarningCount++
}
...
if (Test-Path $VSCodeSettingsPath) {
    try {
        ...
        if ($VSCodeConfig.'claudeCode.selectedModel' -eq "MiniMax-M2-Stable") {
            Write-Status "VS Code selectedModel: MiniMax-M2-Stable" "PASS" "Green"
        } else {
...
if ($ErrorCount -eq 0 -and $WarningCount -eq 0) {
    ...
    Write-Host "`nMiniMax M2-Stable is properly configured." -ForegroundColor Green
    ...
    Write-Host "`nMiniMax M2-Stable should work, but some optional settings are missing." -ForegroundColor Yellow
    ...
    Write-Host "`nMiniMax M2-Stable may NOT work correctly." -ForegroundColor Red
    Write-Host "`nAction required: Fix errors above before using MiniMax M2-Stable." -ForegroundColor Cyan
}
