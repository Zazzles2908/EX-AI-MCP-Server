...
def _emit_provider(event: Dict[str, Any]) -> None:
    try:
        ...
        logger.debug("provider JSONL emit failed")
...
class GLMProvider(BaseProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_url = os.getenv("GLM_API_URL", "https://open.bigmodel.cn/api/paas/v4")
    ...
    async def execute_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Ensure correlation id is present for logging
            req_id = request.get("request_id") or str(uuid.uuid4())
            ...
            # Correlation id & provider call breadcrumb
            req_id = request.get("request_id") or str(uuid.uuid4())
            request["request_id"] = req_id
            _emit_provider({
                "type": "provider_call",
...
class KimiProvider(BaseProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        # Kimi/Moonshot (OpenAI-compatible): prefer KIMI_API_KEY and KIMI_API_URL
        self.api_key = os.getenv("KIMI_API_KEY") or os.getenv("MOONSHOT_API_KEY")
        self.model = self.config.get("KIMI_DEFAULT_MODEL", "moonshot-v1-8k")
