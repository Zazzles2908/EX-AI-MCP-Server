...
class ModelProvider(ABC):
    ...
    def get_preferred_model(self, category: "ToolModelCategory", allowed_models: list[str]) -> Optional[str]:
        ...
        Env-driven preference (future-proof, cost-aware):
        - Reads <PROVIDER>_PREFERRED_MODELS (ordered, comma-separated)
          where PROVIDER is self.get_provider_type().value uppercased (e.g., KIMI, GLM)
        - Returns the first preferred model present in allowed_models
        - Falls back to None if no match

        Args:
            category: The tool category requiring a model (currently not used here)
            allowed_models: Pre-filtered list of model names that are allowed by restrictions

        Returns:
            Preferred model name, or None if no preference found
        """
        try:
            provider_key = self.get_provider_type().value.upper()
            env_var = f"{provider_key}_PREFERRED_MODELS"
            prefs = os.getenv(env_var, "").strip()
            if not prefs:
                return None
            preferred_list = [m.strip() for m in prefs.split(",") if m.strip()]
            # Preserve order; return first that is allowed (case-insensitive match)
            allowed_lower = {m.lower(): m for m in allowed_models}
            for pref in preferred_list:
                if pref.lower() in allowed_lower:
                    return allowed_lower[pref.lower()]
            return None
        except Exception:
            return None
