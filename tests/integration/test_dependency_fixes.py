...
def test_glm_provider_imports():
    ...
    try:
        from src.providers.glm_tool_processor import chat_completions_create_messages_with_session
        assert chat_completions_create_messages_with_session is not None
        print("✅ glm_tool_processor imports successfully")
    except ImportError as e:
        pytest.fail(f"Failed to import glm_tool_processor: {e}")

    # Test that glm_provider module imports without errors (don't check specific function)
    try:
        from src.providers import glm_provider
        assert glm_provider is not None
        print("✅ glm_provider module imports successfully")
    except ImportError as e:
        pytest.fail(f"Failed to import glm_provider module: {e}")
