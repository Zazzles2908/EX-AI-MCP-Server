...
Refactored: Phase 2.1 - Extracted configuration, schema, and validation to separate modules
"""
...
from .consensus_config import CONSENSUS_WORKFLOW_FIELD_DESCRIPTIONS, get_stance_enhanced_prompt
from .consensus_schema import build_consensus_schema
from .consensus_validation import validate_consensus_step_one
...
of the evidence, even when it strongly points in one direction.""",
    ...
    def get_input_schema(self) -> dict[str, Any]:
        ...
        return build_consensus_schema(self.get_name(), self.get_model_field_schema())
    ...
    async def handle_work_completion(self, response_data: dict, request, arguments: dict) -> dict:  # noqa: ARG002
        ...
        response_data["complete_consensus"] = {
            ...
            "models_consulted": [f"{m.get('model','unknown')}:{m.get('stance','neutral')}" for m in self.accumulated_responses],
    ...
    async def execute_workflow(self, arguments: dict[str, Any]) -> list:
        ...
        if request.step_number <= request.total_steps:
            ...
            if model_idx < len(self.models_to_consult):
                ...
                # Add consensus-specific metadata using the actually consulted model
                consulted_model = model_response.get("model", "unknown")
                try:
                    provider = self.get_model_provider(consulted_model)
                ...
                response_data["metadata"] = {
                    ...
                    "model_consulted": f"{consulted_model}:{model_response.get('stance', 'neutral')}",
    ...
    async def _consult_model(self, model_config: dict, request) -> dict:
        ...
        try:
            ...
            # Friendly guidance for missing model context during blinded consensus
            if "Model context not provided" in str(e):
                return {
                    ...
                    "error": (
                        ...
                        "Please call this tool through the server wrapper so the model and context are pre-resolved."
                    ),
                }
    ...
    def _preflight_validate_step_one(self, request) -> None:
        ...
        """
        validate_consensus_step_one(request)
    ...
    def _get_stance_enhanced_prompt(self, stance: str, custom_stance_prompt: str | None = None) -> str:
        ...
        return get_stance_enhanced_prompt(stance, custom_stance_prompt)
