...
class ChallengeTool(SimpleTool):
    ...
    async def execute(self, arguments: dict[str, Any]) -> list:
        ...
        Returns standardized ToolOutput and offers continuation when possible for
        consistent response metadata across tools.
        """
        import json
        import logging
        ...
        try:
            ...
            # Offer continuation when conversation capacity allows
            continuation = self._create_continuation_offer(request, model_info=None)
            if continuation:
                tool_output = self._create_continuation_offer_response(
                    content_str, continuation, request, model_info=None
                )
                return [TextContent(type="text", text=tool_output.model_dump_json())]

            # Fallback: simple success payload
            payload = ToolOutput(
                status="success",
                content=content_str,
                content_type="text",
