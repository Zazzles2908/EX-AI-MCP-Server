"""
Version Tool - Display EX MCP Server version and system information

This tool provides version information about the EX MCP Server including
...
class VersionTool(BaseTool):
    """
    Tool for displaying EX MCP Server version and system information.
    ...
    async def execute(self, arguments: dict[str, Any], on_chunk=None) -> list[TextContent]:
        ...
        # Local build is authoritative; no online update checks
        output_lines.append("## Version Source")
        output_lines.append("This local build is authoritative. Online update checks are disabled.")
        output_lines.append("")
        ...
        try:
            ...
            # Check each provider type (include native Kimi/GLM first, then others)
            provider_types = [
                ProviderType.KIMI,
                ProviderType.GLM,
                ProviderType.GOOGLE,
